...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        neurons_EQ = round(config['neurons_EQ'])
        batch_size_EQ = round(config['batch_size_EQ'])
        epochs_EQ = config['epochs_EQ']
        num_dense_layers = round(config['num_dense_layers'])
        first_decay_steps_EQ = config['first_decay_steps_EQ']
        t_mul_EQ = config['t_mul_EQ']
        alpha_EQ = config['alpha_EQ']
        m_mul_EQ = config['m_mul_EQ']
        initial_learning_rate_EQ = config['initial_learning_rate_EQ']
        l2 = config['l2']
        ...
        BO_CAE_EQ_model = Model(inputs=input_FES, outputs=output_EQ_layer)
        BO_CAE_EQ_model.summary()
        ...
        BO_CAE_EQ_model.compile(optimizer=adam1, loss='mse', metrics=['mse'])
        hist_BO_CAE_EQ = BO_CAE_EQ_model.fit(norm_DataTrainSpec, EQ_Train,
                                           validation_data=(norm_DataValidSpec, EQ_Valid),
                                           epochs=epochs_EQ,
                                           callbacks=[early_stop_callback],
                                           batch_size=batch_size_EQ, shuffle=True, verbose=2) 
        
        BO_scores = hist_BO_CAE_EQ.history['val_loss'][-1]
...
def get_configspace():
        ...
        CSH.UniformFloatHyperparameter('alpha_EQ', lower=0.00001, upper=0.01),
        CSH.UniformIntegerHyperparameter('batch_size_EQ', lower=16, upper=256),
        CSH.UniformFloatHyperparameter('first_decay_steps_EQ', lower=1000, upper=10000),
        CSH.UniformFloatHyperparameter('initial_learning_rate_EQ', lower=0.00001, upper=0.01),
        CSH.UniformFloatHyperparameter('m_mul_EQ', lower=0.5, upper=0.96),
        CSH.UniformIntegerHyperparameter('neurons_EQ', lower=32, upper=512),
        CSH.UniformIntegerHyperparameter('num_dense_layers', lower=0, upper=5),
        CSH.UniformFloatHyperparameter('t_mul_EQ', lower=1, upper=9),
        CSH.UniformIntegerHyperparameter('epochs_EQ', lower=4, upper=64),
...
name_host='127.0.0.1'
NS = hpns.NameServer(run_id='BOBH_CAE_EQ', host=name_host, port=None)
ns_host, ns_port = NS.start()

# Initialize the worker
worker = MyWorker(run_id='BOBH_CAE_EQ',
             host=name_host,
             nameserver=ns_host,
...
results_CAE_EQ = res.get_all_runs()
best_result_CAE_EQ = res.get_incumbent_id()
print(f"Best result: {best_result_CAE_EQ}")

# Get and print the best configuration
incumbent_config_CAE_EQ = res.get_id2config_mapping()[best_result_CAE_EQ]['config']
print(results_CAE_EQ)
print(f"Optimized configuration: {incumbent_config_CAE_EQ}")
