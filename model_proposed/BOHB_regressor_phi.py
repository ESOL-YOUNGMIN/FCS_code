...
#Optimized configuration: {'alpha_2': 0.00021685210099460958, 'batch_size2': 99, 'epochs_2': 20, 'first_decay_steps2': 3474.0062123969005, 'initial_learning_rate2': 0.0004041415719006777, 'l2': 0.0023346660001780733, 'm_mul_2': 0.6491410310644824, 'neurons2': 233, 'num_dense_layers': 1, 't_mul_2': 7.691101807190409}
#Optimized configuration: {'alpha_2': 0.00019966543917363598, 'batch_size2': 87, 'epochs_2': 56, 'first_decay_steps2': 2711.1083579286933, 'initial_learning_rate2': 0.0002214925812245096, 'l2': 0.009840113389233456, 'm_mul_2': 0.7554663790915086, 'neurons2': 174, 'num_dense_layers': 1, 't_mul_2': 6.318370113330396}
...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        neurons2 = round(config['neurons2'])
        batch_size2 = round(config['batch_size2'])
        num_dense_layers = round(config['num_dense_layers'])
        first_decay_steps2 = config['first_decay_steps2']
        t_mul_2 = config['t_mul_2']
        alpha_2 = config['alpha_2']
        m_mul_2 = config['m_mul_2']
        initial_learning_rate2 = config['initial_learning_rate2']
        ...
        BO_EQ_model = Model(inputs=CAE_model.input, outputs=output_EQ_layer)
        BO_EQ_model.summary()
        ...
        BO_EQ_model.compile(optimizer=adam2, loss='mse', metrics=['mse'])
        hist_BO_EQ = BO_EQ_model.fit(norm_DataTrainSpec, EQ_Train,
                                           validation_data=(norm_DataValidSpec, EQ_Valid),
                                           epochs=epochs_2,
                                           callbacks=[early_stop_callback],
                                           batch_size=batch_size2, shuffle=True, verbose=2) 
        
        BO_scores = hist_BO_EQ.history['val_loss'][-1]
        #-r2_score(EQ_Valid, predict_Valid_EQ)
        #predict_Valid_EQ = BO_EQ_model.predict(norm_DataValidSpec)

        print(BO_scores)
      #  print('Valid_r2score',r2_score(EQ_Valid, predict_Valid_EQ))

        return {'loss': BO_scores, 
                'info':{
           #     'Valid_r2score':r2_score(EQ_Valid, predict_Valid_EQ),
                'BO_scores':BO_scores}
                }
#Optimized configuration: {'alpha_2': 8.449006599047595e-05, 'batch_size2': 133, 'epochs_2': 32, 'first_decay_steps2': 4830.062730835012, 'initial_learning_rate2': 0.005632271417731077, 'l2': 0.0002194643379332143, 'm_mul_2': 0.693771030626938, 'neurons2': 228, 'num_dense_layers': 1, 't_mul_2': 7.234048635795383}


# alpha_2 =  0.00015208994316065624
# batch_size2 =  152
# epochs2 = 51
# first_decay_steps2 = 1141.1220427017663
# initial_learning_rate2 = 0.00280254107104870
# l2 =  1.3282851662514498e-06
# m_mul_2 =  0.7506954928665293
# neurons2 = 300
# t_mul_2 =  8.11280571281986
# num_dense_layers = 0
...
def get_configspace():
        ...
        CSH.UniformFloatHyperparameter('alpha_2', lower=0.00001, upper=0.001),
        CSH.UniformIntegerHyperparameter('batch_size2', lower=80, upper=128),
        CSH.UniformFloatHyperparameter('first_decay_steps2', lower=1000, upper=2000),
        CSH.UniformFloatHyperparameter('initial_learning_rate2', lower=0.0001, upper=0.01),
        CSH.UniformFloatHyperparameter('m_mul_2', lower=0.6, upper=0.8),
        CSH.UniformIntegerHyperparameter('neurons2', lower=100, upper=300),
        CSH.UniformIntegerHyperparameter('num_dense_layers', lower=0, upper=3),
        CSH.UniformFloatHyperparameter('t_mul_2', lower=7, upper=9),
        CSH.UniformFloatHyperparameter('l2', lower=1e-09, upper=1e-03 ),
        CSH.UniformIntegerHyperparameter('epochs_2', lower=8, upper=64),
...
name_host='127.0.0.1'
NS = hpns.NameServer(run_id='BOBH_EQ', host=name_host, port=None)
ns_host, ns_port = NS.start()

# Initialize the worker
worker = MyWorker(run_id='BOBH_EQ',
             host=name_host,
             nameserver=ns_host,
...
results_EQ = res.get_all_runs()
best_result_EQ = res.get_incumbent_id()
# Get and print the best configuration
incumbent_config_EQ = res.get_id2config_mapping()[best_result_EQ]['config']
losses = [r.loss for r in results_EQ]

print(results_EQ)
print(f"Best result: {best_result_EQ}")
print(f"Optimized configuration: {incumbent_config_EQ}")
