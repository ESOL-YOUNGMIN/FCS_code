...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        neurons1 = round(config['neurons1'])
        batch_size1 = round(config['batch_size1'])
        epochs_1 = config['epochs_1']
        num_dense_layers = round(config['num_dense_layers'])
        first_decay_steps1 = config['first_decay_steps1']
        t_mul_1 = config['t_mul_1']
        alpha_1 = config['alpha_1']
        m_mul_1 = config['m_mul_1']
        initial_learning_rate1 = config['initial_learning_rate1']
        l2 = config['l2']
        ...
        BO_FR_model = Model(inputs=CAE_model.input, outputs=output_FR_layer)
        BO_FR_model.summary()
        ...
        BO_FR_model.compile(optimizer=adam1, loss='mse', metrics=['mse'])
        hist_BO_FR = BO_FR_model.fit(norm_DataTrainSpec, FR_Train,
                                           validation_data=(norm_DataValidSpec, FR_Valid),
                                           epochs=epochs_1,
                                           callbacks=[early_stop_callback],
                                           batch_size=batch_size1, shuffle=True, verbose=2) 
        
        BO_scores = hist_BO_FR.history['val_loss'][-1]
        print(BO_scores)
...
# alpha_1 = 0.008254542283891697
# batch_size1 = 52
# epochs1 = 28
# first_decay_steps1 = 4693.688061441935
# initial_learning_rate1 =   0.006556878711451283
# l2 = 4.748610305699716e-06
# m_mul_1 = 0.6934659339601337
# neurons1 = 205
# num_dense_layers = 2
# t_mul_1 = 2.4751352652448166

# Define the configuration space
def get_configspace():
        config_space = CS.ConfigurationSpace()
        config_space.add_hyperparameters([
        CSH.UniformFloatHyperparameter('alpha_1', lower=0.00001, upper=0.01),
        CSH.UniformIntegerHyperparameter('batch_size1', lower=16, upper=256),
        CSH.UniformFloatHyperparameter('first_decay_steps1', lower=2000, upper=6000),
        CSH.UniformFloatHyperparameter('initial_learning_rate1', lower=0.00001, upper=0.01),
        CSH.UniformFloatHyperparameter('m_mul_1', lower=0.6, upper=0.8),
        CSH.UniformIntegerHyperparameter('neurons1', lower=100, upper=300),
        CSH.UniformIntegerHyperparameter('num_dense_layers', lower=0, upper=3),
        CSH.UniformFloatHyperparameter('t_mul_1', lower=1, upper=5),
        CSH.UniformIntegerHyperparameter('epochs_1', lower=4, upper=64),
...
name_host='127.0.0.1'
NS = hpns.NameServer(run_id='BOBH_FR', host=name_host, port=None)
ns_host, ns_port = NS.start()

# Initialize the worker
worker = MyWorker(run_id='BOBH_FR',
             host=name_host,
             nameserver=ns_host,
...
results_FR = res.get_all_runs()
best_result_FR = res.get_incumbent_id()
print(f"Best result: {best_result_FR}")

# Get and print the best configuration
incumbent_config_FR = res.get_id2config_mapping()[best_result_FR]['config']
print(results_FR)
print(f"Optimized configuration: {incumbent_config_FR}")

losses = [r.loss for r in results_FR]
