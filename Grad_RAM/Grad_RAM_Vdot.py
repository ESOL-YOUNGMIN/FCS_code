...
FR_model = tf.keras.models.load_model(checkpoint_path_combine_FR)
FR_model.summary()
...
grad_ram_nosum = np.zeros(1600*num).reshape(1600,num)
mean_grad_ram = np.zeros(1600)

for FF in index_FF:
    for index in range(0, num):
        print(index)
        filename0 = os.path.join(file_path_spectrum, str(flowRate[FF]), str(equiRatio[3]), str(mixRatio[0]), "*.xlsx")
        d = os.listdir(os.path.dirname(filename0))
        filename = os.path.join(file_path_spectrum, str(flowRate[FF]), str(equiRatio[3]), str(mixRatio[0]), d[0])
        ...
        grad_model = tf.keras.models.Model([FR_model.inputs],
                                           [FR_model.get_layer('encoder_2').output, FR_model.output])
...
grad_neg = pd.DataFrame(grad_neg)
grad_pos = pd.DataFrame(grad_pos)
normal_spectra = pd.DataFrame(normal_spectra)
grad_neg.to_excel('./output/gard_neg_FR_0.85_0.xlsx', index=False)
grad_pos.to_excel('./output/gard_pos_FR_0.85_0.xlsx', index=False)
normal_spectra.to_excel('./output/normal_spectra_FR_0.85_0.xlsx', index=False)
