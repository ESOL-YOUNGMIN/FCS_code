...
EQ_model = tf.keras.models.load_model(checkpoint_path_combine_EQ)
EQ_model.summary()
combined_model = EQ_model
...
for EE in index_EE:
    ...
    for index in range(0, num):
        print(index)
        filename0 = os.path.join(file_path_spectrum, str(flowRate[3]), str(equiRatio[EE]), str(mixRatio[0]), "*.xlsx")
        d = os.listdir(os.path.dirname(filename0))
        filename = os.path.join(file_path_spectrum, str(flowRate[3]), str(equiRatio[EE]), str(mixRatio[0]), d[0])
        spectra = pd.read_excel(filename, header=None).values
        spectra = spectra[5:1605, 10*index+1]
        ...
        grad_model = tf.keras.models.Model([combined_model.inputs], [combined_model.get_layer('encoder_2').output, combined_model.output])

        with tf.GradientTape(persistent=True) as tape:
            tape.watch(x)
            last_conv_layer_output, preds = grad_model(x)
        weights = combined_model.get_layer('encoder_2').get_weights()
